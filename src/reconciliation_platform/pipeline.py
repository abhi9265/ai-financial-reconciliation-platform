...
from reconciliation_platform.anomaly.detector import Anomaly, detect_anomalies
...
from reconciliation_platform.reconciliation.engine import ReconciliationDecision, reconcile
from reconciliation_platform.validation.rules import QualityIssue, validate_transactions
...
def run_pipeline(data_dir: str | Path) -> dict:
    ...
    bank = normalize_rows(bank_rows, context=NormalizationContext(SourceSystem.BANK, bank_path.name, bank_hash, SCHEMA_VERSION, bank_batch, now))
    purchase = normalize_rows(purchase_rows, context=NormalizationContext(SourceSystem.PURCHASE_REGISTER, purchase_path.name, purchase_hash, SCHEMA_VERSION, purchase_batch, now))
    ...
    return {
        "bank": bank, "purchase": purchase, "quality_issues": quality,
        "decisions": decisions, "anomalies": anomalies,
        "metadata": {
            "bank_file_hash": bank_hash, "purchase_file_hash": purchase_hash,
            "bank_batch_id": bank_batch, "purchase_batch_id": purchase_batch,
            "bank_rows": len(bank), "purchase_rows": len(purchase),
        },
    }
